...
join reviews on series_id=reviewer_id;
...
join reviews on series_id=reviewer_id group by title; 
...
select first_name, last_name, count(rating) count, min(rating) min, max(rating) max, avg(rating) avg, 
CASE
...
END AS 'STATUS'
from reviewers
join reviews on reviews.reviewer_id=reviewers.id group by reviewers.id ;
...
select title, rating, concat(first_name,' ',last_name) 'reviewer' from series
